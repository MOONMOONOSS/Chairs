...
public class Chairs extends JavaPlugin {
	...
	protected Chairs(JavaPluginLoader loader, PluginDescriptionFile description, File dataFolder, File file) {
		instance = this;
	}

	public static Entity spawnChairsArrow(final Location location) {
		final Arrow arrow = location.getWorld().spawnArrow(location, new Vector(), 0, 0);
	...
	protected void saveSitDisabled() {
		try {
			...
			try (final PrintWriter writer = new PrintWriter(sitDisabledFile, "UTF-8")) {
