...
public class ChairEffects {
	...
	public ChairEffects(final Chairs plugin) {
		this.plugin = plugin;
	...
	protected void startHealing() {
		healTaskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(
			...
			() ->
				...
				.filter(final p -> p.hasPermission("chairs.sit.health"))
						...
						if (entity instanceof Item) {
							...
							if (item.getPickupDelay() == 0) {
								if (p.getInventory().firstEmpty() != -1) {
									final EntityPickupItemEvent pickupevent = new final EntityPickupItemEvent(p, item, 0);
							...
							while (exptoadd > 0) {
								...
								if (p.getExpToLevel() < exptoadd) {
									...
									final PlayerExpChangeEvent expchangeevent = new final PlayerExpChangeEvent(p, localexptoadd);
									Bukkit.getPluginManager().callEvent(expchangeevent);
									p.giveExp(expchangeevent.getAmount());
									if (p.getExpToLevel() <= 0) {
										final PlayerLevelChangeEvent levelchangeevent = new final PlayerLevelChangeEvent(p, p.getLevel(), p.getLevel()+1);
